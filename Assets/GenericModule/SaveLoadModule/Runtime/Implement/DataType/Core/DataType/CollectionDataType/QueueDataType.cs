	...
	public class QueueDataType : CollectionDataType
	{
		...
		public override void ReadInto<T>(IReader reader, object obj)
		{
			if (reader.StartReadCollection())
				throw new NullReferenceException("[QueueDataType::ReadInto] The Collection we are trying to load is stored as null, which is not allowed when using ReadInto methods.");
			...
			foreach (var item in queue)
			{
				...
				if (itemsLoaded == queue.Count)
					throw new IndexOutOfRangeException("[QueueDataType::ReadInto] The collection we are loading is longer than the collection provided as a parameter.");
			}

			// If we loaded fewer items than the parameter collection, throw index out of range exception.
			if (itemsLoaded != queue.Count)
				throw new IndexOutOfRangeException("[QueueDataType::ReadInto] The collection we are loading is shorter than the collection provided as a parameter.");
