	...
	public class StackDataType : CollectionDataType
	{
		...
		public override void ReadInto<T>(IReader reader, object obj)
		{
			if (reader.StartReadCollection())
				throw new NullReferenceException("[StackDataType::ReadInto] The Collection we are trying to load is stored as null, which is not allowed when using ReadInto methods.");
			...
			foreach (var item in stack)
			{
				...
				if (itemsLoaded == stack.Count)
					throw new IndexOutOfRangeException("[StackDataType::ReadInto] The collection we are loading is longer than the collection provided as a parameter.");
			}

			// If we loaded fewer items than the parameter collection, throw index out of range exception.
			if (itemsLoaded != stack.Count)
				throw new IndexOutOfRangeException("[StackDataType::ReadInto] The collection we are loading is shorter than the collection provided as a parameter.");
		...
		public override object Read(IReader reader)
		{
			...
			_reflectionHelper.GetMethods(instance.GetType(), "Reverse").FirstOrDefault(t => !t.IsStatic).Invoke(instance, new object[] { });
