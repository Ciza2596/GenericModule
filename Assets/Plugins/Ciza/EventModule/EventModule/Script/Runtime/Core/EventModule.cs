...
{
	public abstract class EventModule<TBaseEvent, TSyncEvent, TAsyncEvent> where TSyncEvent : TBaseEvent where TAsyncEvent : TBaseEvent
	{
		//private method
		private readonly Dictionary<Type, SyncEventDelegateContainer>  _syncEvents  = new Dictionary<Type, SyncEventDelegateContainer>();
		private readonly Dictionary<Type, AsyncEventDelegateContainer> _asyncEvents = new Dictionary<Type, AsyncEventDelegateContainer>();
		...
		//async
		public void AddAsyncListener<TEvent>(Func<TEvent, CancellationToken, UniTask> eventDelegate) where TEvent : TAsyncEvent =>
			AddListener<TEvent, AsyncEventDelegateContainer>(eventDelegate, _asyncEvents);

		public void RemoveAsyncListener<TEvent>(Func<TEvent, CancellationToken, UniTask> eventDelegate) where TEvent : TAsyncEvent =>
			RemoveListener<TEvent, AsyncEventDelegateContainer>(eventDelegate, _asyncEvents);

		public async UniTask NotifyAsyncEvent<TEvent>(TEvent eventData, CancellationToken cancellationToken = default) where TEvent : TAsyncEvent
		{
		...
		public UniTask NotifyAsyncEvent<TEvent>(TEvent eventData, out CancellationTokenSource notifyAsyncEventCTS) where TEvent : TAsyncEvent
		{
		...
		//private method
		private void AddListener<TEvent, TEventDelegateContainer>(Delegate eventDelegate, Dictionary<Type, TEventDelegateContainer> events) where TEvent : TBaseEvent where TEventDelegateContainer : BaseEventDelegateContainer, new()
		{
		...
		private void RemoveListener<TEvent, TEventDelegateContainer>(Delegate eventDelegate, Dictionary<Type, TEventDelegateContainer> events) where TEvent : TBaseEvent where TEventDelegateContainer : BaseEventDelegateContainer, new()
		{
			var type = typeof(TEvent);
