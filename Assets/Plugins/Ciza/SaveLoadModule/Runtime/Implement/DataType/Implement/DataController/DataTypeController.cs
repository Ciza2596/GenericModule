...
namespace DataType.Implement
{
	...
	{
		//private variable
		private readonly Dictionary<Type, BaseDataType> _dataTypes = new Dictionary<Type, BaseDataType>();
		...
		public BaseDataType GetOrCreateDataType(Type key)
		{
		...
		private BaseDataType CreateDataType(Type type, IDataTypeController dataTypeController, IReflectionHelper reflectionHelper)
		{
			...
			else if (reflectionHelper.CheckIsImplementsInterface(type, typeof(IEnumerable)) && reflectionHelper.CheckIsGenericType(type))
			{
