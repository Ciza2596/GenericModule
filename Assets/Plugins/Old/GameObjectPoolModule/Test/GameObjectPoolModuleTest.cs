...
public class GameObjectPoolModuleTest
{
	...
	private const string _prefabNamePrefix = "Prefab";
	...
	public void SetUp()
	{
		...
		_gameObjectPoolModule       = new GameObjectPoolModule(_gameObjectPoolModuleConfig);
	...
	//private method
	private Dictionary<string, GameObject> CreatePrefabMap(int length, string keyPrefix, string prefabNamePrefix)
	{
	...
	private void Check_GameObjectPoolModule_Is_Initialized() =>
		Assert.IsTrue(_gameObjectPoolModule.IsInitialized, "GameObjectPoolModule is Uninitialized.");

	private void Check_GameObjectPoolModule_Is_Uninitialized() =>
		Assert.IsFalse(_gameObjectPoolModule.IsInitialized, "GameObjectPoolModule is initialized.");
	...
	private void Check_SpawnTransform_Hasnt_Children(int count = 0) =>
		Assert.IsTrue(_spawnTransform.childCount <= count, $"SpawnTransform has children.");
	...
	private GameObject Spawn_GameObject_To_SpawnTransform(int keyIndex, Dictionary<string, GameObject> prefabMap)
	{
		var key = prefabMap.Keys.ToArray()[keyIndex];
		return _gameObjectPoolModule.Spawn(key, parentTransform: _spawnTransform);
	}

	private GameObject[] Spawn_GameObjects_To_SpawnTransform(int keyIndex, int count, Dictionary<string, GameObject> prefabMap)
	{
