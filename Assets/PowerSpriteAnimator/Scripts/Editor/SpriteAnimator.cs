...
namespace PowerTools
{
	public partial class SpriteAnimator : EditorWindow
	{
		...
		[SerializeField]
		bool m_uiImage = false;

		[SerializeField]
		List<AnimFrame> m_frames = null;

		[SerializeField]
		List<AnimEvent> m_events = null;

		[SerializeField]
		string m_spritePath = string.Empty;

		[SerializeField]
		float m_infoPanelWidth = 260;
		...
		[SerializeField]
		SendMessageOptions m_defaultEventMessageOptions = SendMessageOptions.DontRequireReceiver;

		[SerializeField]
		bool m_defaultEventSendUpwards = false;

		[SerializeField]
		bool m_defaultEventUsePrefix = false;

		[SerializeField]
		string[] m_defaultNodeNames = new string[SpriteAnimNodes.NUM_NODES];

		[SerializeField]
		Sprite[] m_defaultNodeSprites = new Sprite[SpriteAnimNodes.NUM_NODES];
		...
		[SerializeField]
		float m_defaultFrameLength = 0.1f;

		[SerializeField]
		int m_defaultFrameSamples = 6;

		[SerializeField]
		bool m_ignorePivot = false;
		...
		[SerializeField]
		List<AnimFrame> m_copiedFrames = null;

		[SerializeField]
		List<AnimEvent> m_copiedEvents = null;
		...
		void OnClipChange(bool resetPreview = true)
		{
			...
			if (m_curveBinding.isPPtrCurve)
			{
				...
				m_frames = new List<AnimFrame>(
				                               System.Array.ConvertAll<ObjectReferenceKeyframe, AnimFrame>(objRefKeyframes, keyframe => { return new AnimFrame() { m_time = keyframe.time, m_sprite = keyframe.value as Sprite }; }));
			}
		...
		void InsertFrames(Sprite[] sprites, int atPos)
		{
			...
			AnimFrame[] newFrames = System.Array.ConvertAll<Sprite, AnimFrame>(sprites, sprite => { return new AnimFrame() { m_sprite = sprite, m_length = frameLength }; });
			...
			AnimEvent newEvent = new AnimEvent()
			                     {
				                     m_time           = eventTime,
				                     m_functionName   = "",
				                     m_messageOptions = m_defaultEventMessageOptions,
				                     m_usePrefix      = m_defaultEventUsePrefix,
				                     m_sendUpwards    = m_defaultEventSendUpwards
			                     };
